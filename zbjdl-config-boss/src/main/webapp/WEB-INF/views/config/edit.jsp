        ...
        <div class="content_main">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form id="postForm" id="save_form" action="update" method="post"
...
<!--                         <div class="col-lg-offset-2 col-lg-10">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-lg-6">
                                ...
                                <label class="control-label">${config.namespace}&nbsp;</label>
                  <input type="hidden" name="config.namespace" value="${config.namespace}"></input>
                  ...
                  <input type="hidden" name="config.type" value="${config.type}"></input>
						...
						<div class="form-group">
							...
							<c:if test="${config.valueTypeStr eq 'VALUE'}">
								<div class="col-lg-6" id="primiterType">
									<!-- 修改布尔型每次保存都会增加双引号的问题 -->
									<textarea class="form-control" name="config.value" rows="1"><c:if
											test="${config.valueDataTypeStr != 'BOOLEAN'}"> <c:out
												value='${config.value}' />
										</c:if><c:if test="${config.valueDataTypeStr eq 'BOOLEAN'}">${config.value}</c:if></textarea>
								</div>
							</c:if>
											...
											<c:forEach items="${entityMap}" var="entity"
												...
												<tr>
													<td><input class="input_text" type="text"
														name="itemKey" value="<c:out value='${entity.key}'/>"
														label="键" req="true" len="{max:100}" /></td>
													<td><input class="input_text" type="text"
														name="itemValue"
														<c:if test="${entityDataType != 'BOOLEAN'}">value="<c:out value='${entity.value}'/>"</c:if>
														label="值" req="true" len="{max:10000}"
														style="width: 250px"
														<c:if test="${entityDataType eq 'BOOLEAN'}">bool="${entity.value}"</c:if> /></td>
													<td><select class="select" name="itemType"
														onchange="itemTypeChange(this);">
															<option value="STRING"
																<c:if test="${entityDataType eq 'STRING'}">selected="selected"</c:if>><fmt:message
																	key="enum_value_data_type_STRING" /></option>
															<option value="INTEGER"
																<c:if test="${entityDataType eq 'INTEGER'}">selected="selected"</c:if>><fmt:message
																	key="enum_value_data_type_INTEGER" /></option>
															<option value="BOOLEAN"
																<c:if test="${entityDataType eq 'BOOLEAN'}">selected="selected"</c:if>><fmt:message
																	key="enum_value_data_type_BOOLEAN" /></option>
															<option value="DOUBLE"
																<c:if test="${entityDataType eq 'DOUBLE'}">selected="selected"</c:if>><fmt:message
																	key="enum_value_data_type_DOUBLE" /></option>
															<option value="DATE"
																<c:if test="${entityDataType eq 'DATE'}">selected="selected"</c:if>><fmt:message
																	key="enum_value_data_type_DATE" /></option>
															<option value="DATETIME"
																<c:if test="${entityDataType eq 'DATETIME'}">selected="selected"</c:if>><fmt:message
																	key="enum_value_data_type_DATETIME" /></option>
													</select></td>
													<td><c:choose>
															<c:when test="${status.index == 0}">
																<a name="addTr_a" href="javascript:void(0);"
																	rmdatatype="true">添加</a>
															</c:when>
															<c:otherwise>
									...
									<div id="addMapType">
										<table border="1px" style="border-collapse: collapse;">
											...
											<c:forEach items="${map}" var="entry" varStatus="status">
												<tr>
													<td><input class="input_text" type="text"
														name="mapKey" value='<c:out value="${entry.key}"/>'
														label="键" req="true" len="{max:100}" /></td>
													<td><input class="input_text" type="text"
														name="mapValue"
														<c:if test="${config.valueDataTypeStr != 'BOOLEAN'}">value="<c:out value='${entry.value}'/>"</c:if>
														label="值" req="true" len="{max:10000}"
														style="width: 250px"
														<c:if test="${config.valueDataTypeStr eq 'BOOLEAN'}">bool="${entry.value}"</c:if> /></td>
													<td><c:choose>
															<c:when test="${status.index == 0}">
																<a name="addTr_a" href="javascript:void(0);">添加</a>
															</c:when>
															<c:otherwise>
																<a href='#' onclick='deleteTr(this)'>删除</a>
															</c:otherwise>
										...
										<table border="1px" style="border-collapse: collapse;">
											...
											<c:forEach items="${list}" var="ls" varStatus="status">
												<tr>
													<td><input class="input_text" type="text"
														name="listValue"
														<c:if test="${config.valueDataTypeStr != 'BOOLEAN'}">value="<c:out value='${ls}'/>"</c:if>
														label="值" req="true" len="{max:10000}"
														style="width: 250px"
														<c:if test="${config.valueDataTypeStr eq 'BOOLEAN'}">bool="${ls}"</c:if> /></td>
													<td><c:choose>
															<c:when test="${status.index == 0}">
																<a name="addTr_a" href="javascript:void(0);">添加</a>
															</c:when>
