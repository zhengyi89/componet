 ...
 *   int totalCount  = pg.getTotalCount();       // 总共有几项
 ...
 *
 *   // 还可以做调整。
 *   setPageSize(20);                   // 这样就每页显示20个了，当前页的值会自动调整,
...
public class Paginator implements Serializable, Cloneable {
	...
	public int getLength() {
		if (page > 0) {
			return Math.min(pageSize * page, totalCount)
					- (pageSize * (page - 1));
		} else {
			return 0;
		}
	...
	public int setOffset(int itemOffset) {
		return setPage((itemOffset / pageSize) + 1);
	}
	...
	public String toString() {
		...
		sb.append("    Showing totalCount ").append(getBeginIndex())
				.append(" to ").append(getEndIndex()).append(" (total ")
				.append(getTotalCount()).append(" totalCount), ");
